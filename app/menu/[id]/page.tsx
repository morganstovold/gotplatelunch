...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: ["Hawaiian food", "plate lunch", menuItem.name.toLowerCase(), ...(menuItem.dietaryInfo || []), menuItem.category.toLowerCase()],
    alternates: {
  ...
  return (
    <main className="flex flex-col items-center relative min-h-screen">
      ...
      <section className="container max-w-5xl py-12 sm:py-16 md:py-20 lg:py-24">
        <div className="mb-8">
          <Link href="/menu" className="flex items-center text-gray-600 hover:text-black transition-colors">
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {menuItem.dietaryInfo.map((info) => (
                    <span key={info} className="bg-blue-50 text-blue-800 px-3 py-1 rounded-full text-sm">
                      {info}
                    </span>
                  ))}
                </div>
